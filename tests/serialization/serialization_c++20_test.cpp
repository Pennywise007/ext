...
struct PrivateFields
{
    ...
    static PrivateFields Create()
    {
        PrivateFields obj;
        obj.publicField = 123;
        obj.privateField = 789;

        [[maybe_unused]] auto&& [t1, t2] = obj;
...
private:
   ...
   // template <typename Type, size_t Index, size_t End>
    //friend struct find_max_brace_constructor;


    template <typename Type, size_t Index>
    friend struct ext::reflection::details::make_constructor_signature_impl;

    template <class T, class... TArgs>
    friend decltype(void(T{std::declval<TArgs>()...}), std::true_type{}) 
        ext::reflection::details::test_is_braces_constructible(int);

    template <typename Type, size_t Index, size_t End>
    friend struct ext::reflection::details::find_max_brace_constructor;

    template <template <typename...> class Function, typename List>
    friend struct ext::mpl::apply_impl;
};

TEST(serialization_c_plus_plus_20_test, private_fields)
{
    constexpr auto f = ext::reflection::fields_count<PrivateFields>;

    auto obj = PrivateFields::Create();
    //[[maybe_unused]] auto&& [t1] = obj;
