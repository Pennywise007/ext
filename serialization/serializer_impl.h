...
#ifdef USE_PUGI_XML
inline std::unique_ptr<serializer::ISerializer> Fabric::XMLSerializer(const std::filesystem::path& filePath)
{
    return std::make_unique<SerializerXML>(filePath);
}

inline std::unique_ptr<serializer::IDeserializer> Fabric::XMLDeserializer(const std::filesystem::path& filePath)
{
    return std::make_unique<SerializerXML>(filePath);
}
#endif

inline std::unique_ptr<serializer::ISerializer> Fabric::TextSerializer(std::wstring& outputText)
{
    return std::make_unique<SerializerText>(&outputText);
}

inline std::unique_ptr<serializer::IDeserializer> Fabric::TextDeserializer(const std::wstring& inputText)
{
...
inline size_t Visitor::GetIndexAmongIdenticalNames(bool bCollectionStart)
{
    ...
    if ((!bCollectionStart && !m_collectionsDepth.empty()) || (bCollectionStart && m_collectionsDepth.size() > 1))
    {
        ...
        for (size_t index = 0; index < collectionInfoIt->currentIndexInsideCollection; ++index)
        {
            ...
            {
                if (_wcsicmp(object->GetName(), curObjectName) == 0)
                    ++counter;
            }
        }
    }
