...
private:
    ...
    try
    {
        ...
        outputStringPrefix << std::string_sprintf("%02d:%02d:%02d.%03d", currentTime.wHour, currentTime.wMinute,
                                                  currentTime.wSecond, currentTime.wMilliseconds).c_str() << '\t'
            << std::hex << GetCurrentThreadId() << '\t' << type << '\t';
