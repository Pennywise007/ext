...
template<class T>
_INLINE_VAR constexpr bool is_pair_v = is_pair<T>::value;
...
template<class T>
_INLINE_VAR constexpr bool is_array_v = is_array<T>::value;
...
_INLINE_VAR constexpr bool is_based_on = std::is_base_of_v<Base, std::remove_pointer_t<std::extract_value_type_v<T>>>;
template<class T>
...
template <class Type>
std::shared_ptr<ISerializable> get_as_serializable(const std::wstring& name, Type* type);
...
{
    SerializableBase(const std::wstring& name, Type* typePointer) : m_name(name), m_typePointer(typePointer) {}
...
{
    SerializableValueHolder(const std::wstring& name, SerializableValue&& _value) : m_name(name), value(std::move(_value)) {}
// ISerializable
...
protected:
// ISerializable
    EXT_NODISCARD const wchar_t* GetName() const EXT_NOEXCEPT override
    {
...
struct SerializableOptional : SerializableBase<ISerializableOptional, Type>
{
    ...
    SerializableOptional(const std::wstring& name, Type* typePointer) : Base(name, typePointer) {}
protected:
// ISerializable
...
{
    using Base = SerializableBase<ISerializableOptional, Type>;
    SerializableValueProxy(const std::wstring& name, Type* typePointer) : Base(name, typePointer) {}
...
template <class Type>
std::shared_ptr<ISerializable> get_serializable(const std::wstring& name, const Type* value)
{
...
{
    SerializableCollectionImpl(std::wstring name) : m_name(std::move(name)) {}
...
struct SerializableMap : SerializableBase<ISerializableCollection, MapType>
{
    ...
    SerializableMap(const std::wstring& name, MapType* typePointer) : Base(name, typePointer) {}
protected:
// ISerializable
...
// ISerializableCollection
    ...
    EXT_NODISCARD std::shared_ptr<ISerializable> Get(const size_t& index) const override
    {
        ...
        auto collection = std::make_shared<SerializableCollectionImpl>(L"Map pair");

        auto it = std::next(container->begin(), index);
        if constexpr (is_serializable_v<typename MapType::key_type>)
            collection->AddField(get_serializable(L"Key", &it->first));
        else
            collection->AddField(std::make_shared<SerializableValueHolder>(L"Key", serialize_value(it->first)));

        collection->AddField(get_as_serializable(L"Value", &it->second));
...
struct SerializableSet : SerializableBase<ISerializableCollection, SetType>
{
    ...
    SerializableSet(const std::wstring& name, SetType* typePointer) : Base(name, typePointer) {}
protected:
// ISerializable
    void PrepareToDeserialize(_Inout_ std::shared_ptr<SerializableNode>& deserializableTree) EXT_THROWS() override
    {
...
// ISerializableCollection
    ...
    EXT_NODISCARD std::shared_ptr<ISerializable> Get(const size_t& index) const override
    {
        ...
        auto it = std::next(container->begin(), index);
        if constexpr (is_serializable_v<typename SetType::value_type>)
            return get_serializable(L"Value", &*it);
        else
            // if set types if not serializable - store string value presentation as index name with empty value
            return std::make_shared<SerializableValueHolder>(L"Value", serialize_value(*it));
    }
};
...
struct SerializableArray : SerializableBase<ISerializableCollection, ArrayType>
{
    ...
    SerializableArray(const std::wstring& name, ArrayType* typePointer) : Base(name, typePointer) {}
protected:
// ISerializable
    void PrepareToDeserialize(_Inout_ std::shared_ptr<SerializableNode>& deserializableTree) EXT_THROWS() override
    {
...
// ISerializableCollection
    ...
    EXT_NODISCARD std::shared_ptr<ISerializable> Get(const size_t& index) const override
    {
        ...
        return get_as_serializable<typename ArrayType::value_type>(serialize_value(index), &*std::next(array->begin(), index));
    }
};
...
struct SerializableField : SerializableBase<ISerializableField, FieldType>
{
    ...
    SerializableField(const std::wstring& name, FieldType* typePointer) : Base(name, typePointer) {}
...
std::shared_ptr<ISerializable> get_as_serializable(const std::wstring& name, Type* type)
{
    ...
    else if constexpr (impl::is_pair_v<Type>)
    {
        ...
        collection->AddField(get_as_serializable<decltype(Type::first)>(L"First", &type->first));
        collection->AddField(get_as_serializable<decltype(Type::second)>(L"Second", &type->second));
        return std::move(collection);
    }
