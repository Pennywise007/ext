...
struct Event : private ext::NonCopyable
{
    ...
    void Create(bool manualReset = false) SSH_THROWS(std::system_error)
    {
        SSH_ASSERT(!m_handle) << "Handle already exist";
    ...
    void Set() SSH_THROWS(std::system_error, ::ext::check::CheckFailedException)
    {
        SSH_EXPECT(m_handle) << "Handle uninitialized";
        if (!::SetEvent(m_handle))
            throw std::system_error(::GetLastError(), std::system_category(), "Failed to create event");
    }

    // Reset event state, set the event state to nonsignaled
    void Reset() SSH_THROWS(std::system_error)
    {
        SSH_EXPECT(m_handle) << "Handle uninitialized";
    ...
    bool Wait(const std::optional<std::chrono::steady_clock::duration>& timeout = std::nullopt) const SSH_THROWS(std::system_error, ::ext::check::CheckFailedException)
    {
    ...
    /// <param name="timeout">Waiting timeout, infinite if not installed</param>
    /// <returns> true if signal raised, false if timeout expired</returns>
    SSH_NODISCARD bool Raised() const SSH_THROWS(std::system_error, ::ext::check::CheckFailedException)
    {
        if (m_handle)
    ...
    constexpr SSH_NODISCARD bool operator!() const SSH_NOEXCEPT { return !m_handle; }
    constexpr SSH_NODISCARD operator bool() const SSH_NOEXCEPT { return m_handle; }

    // Get event handle
    constexpr SSH_NODISCARD HANDLE& GetHandle() { return m_handle; }
    constexpr SSH_NODISCARD const HANDLE& GetHandle() const { return m_handle; }
